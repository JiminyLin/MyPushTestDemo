 ...
 *  setupWithOption:launchingOption:appKey:channel是新版本方法,appKey和channel以及APS_FOR_PRODUCTION只需通过该函数参数传递,不再需要PushConfig.plist文件
 */
+ (void)setupWithOption:(NSDictionary *)launchingOption __attribute__((deprecated("JPush 2.1.0 版本已过期")));// 初始化
+ (void)setupWithOption:(NSDictionary *)launchingOption appKey:(NSString *)appKey channel:(NSString *)channel apsForProduction:(BOOL)isProduction; // 初始化
